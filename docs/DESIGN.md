...
> 实现状态：M1 引擎 / M2 配置同步 / M3 GUI / M4 发布·多机同步 / M5 系统管理与专业工具 / M6 局域网剪贴板同步 **均已实现**（见 git 历史与 README）。
...
- **FR-11 局域网剪贴板同步**：发现局域网内同样运行本应用的设备，经发起方提供的 PIN 配对后，端到端加密共享剪贴板内容（文本 + 图片）——双向同步、预览、集中管理（新增 / 删除即时同步）。开源版默认 2 台上限；传输层抽象，付费版可经服务器中转解除上限并跨网。详见 §15。
...
| **M5 系统管理与专业工具** | 系统概览 / 维护、WSL、系统调优、高级工具、服务 / FTP / Cloudflare DDNS（开发人员模式门控） | 专业运维能力 |
| **M6 局域网剪贴板同步** | UDP 发现 + PIN 配对（PBKDF2 / HMAC）+ AES-GCM 加密链路 + 文本/图片共享板（见 §15） | 多设备即时共享剪贴板 |
...
## 15. 局域网剪贴板同步（FR-11 · M6）

发现局域网内**同样运行本应用**的设备，经 PIN 配对后端到端加密共享剪贴板（文本 + 图片）：双向同步、预览、集中管理。引擎全部在 App 层（`Services/Clip`，与 FTP 服务端同构——网络型功能内聚于 App，不污染 Core），协议 / 加密类保持 UI 无关以便单测与未来下沉。入口在「开发工具」组、**开发人员模式门控**（与 FTP / Cloudflare 一致，因其监听端口）。

### 15.1 模块与依赖

| 组件 | 文件 | 职责 |
|---|---|---|
| 发现 | `PeerDiscovery` | UDP 多播（`239.255.71.84`，TTL 1）周期信标：广播 `id/设备名/TCP端口/版本`，监听并维护在线 peer 列表（仅暴露在线状态，**不经此通道传任何剪贴板数据**）。 |
| 配对 | `ClipLink`（握手部分）+ `ClipPinDialog` | 发起方拨号、加入方应答；PIN 双向证明 + 会话密钥派生。 |
| 加密 | `ClipCrypto` | PIN → PBKDF2 派生密钥；HMAC 挑战应答；HKDF 派生会话密钥；AES-256-GCM 封包。 |
| 传输 | `ClipProtocol` + `ClipLink` | 4 字节大端长度前缀分帧；握手为明文 JSON 帧，配对后为 AES-GCM 密文帧。 |
| 监听 | `ClipboardMonitor` | Win32 `AddClipboardFormatListener`（消息专用窗口，事件驱动非轮询）捕获本机文本 / 图片；带回声抑制。 |
| 编排 | `ClipSyncManager` | 拥有发现 / 监听器 / TcpListener / 链路集 / 共享板；强制设备上限；应用「自动镜像」「落盘」开关；按 id 去重的 mesh 转发，使 3+ 设备一致。 |
| 持久化 | `ClipConfigStore` | 设置 → `clipsync.json`；**仅在用户开启时**历史 → `clipsync-history.json`（设置永不含剪贴板内容）。 |
| UI | `ViewModels/Clip` · `Views/Clip` | 三 Tab：设备与配对（发现 / 邀请 / 启停 / 日志）、共享剪贴板（板 + 预览 + 集中管理）、设置。 |

### 15.2 PIN 配对握手（PIN 永不上线）

> 发起方 A 生成并**屏幕显示** 6 位 PIN（口头告知 B），A 拨号连接 B；B 收到入站连接后被提示输入 PIN（同一连接最多重试 5 次）。

1. **A→B** `Hello{ id_A, salt, nonceA }`（明文；salt/nonce 为公开值）。
2. **B**（用户输入 PIN）`K = PBKDF2(PIN, salt)`；**B→A** `AuthB{ id_B, nonceB, macB = HMAC_K("B"‖nonceA‖nonceB‖id_A‖id_B) }`。
3. **A** 校验 `macB`：失败回 `AuthA{Ok=false}`（B 重试）；成功回 `AuthA{Ok=true, macA = HMAC_K("A"‖…)}`。
4. **B** 校验 `macA`；双方 `SK = HKDF(K, nonceA‖nonceB)`，此后全程 AES-256-GCM。

**威胁模型**：LAN 含被动嗅探 + 可能的主动攻击；PIN 短时有效、尝试次数受限。PIN 不上线，错误 PIN 仅使 MAC 失败、不泄露任何信息；在线穷举受重试上限与邀请时窗约束。**后续硬化**：以真正的 PAKE（SPAKE2 / J-PAKE）替换，进一步抵御「在线猜 PIN 的主动中间人」。

### 15.3 数据流与一致性

- **本机复制** → `ClipboardMonitor` 捕获 → 标记来源 → 入板 → 广播 `Entry` 给所有链路。
- **收到 `Entry`** → 按 `Id` 去重（`_seenIds`）→ 入板 →（可选）镜像到本机剪贴板（抑制回声）→ 转发给其它链路（mesh）。
- **删除** → 本地移除 + 广播 `Delete`；新链路建立时互发 `Board` 全量做合并。
- **回声抑制**：镜像到本机前 `Suppress(contentHash)`，避免「写本机 → 触发监听 → 再广播」的环。

### 15.4 安全与边界

- 凭据/会话密钥**永不入库**；PIN 仅驻留内存且短时有效。
- 剪贴板内容**默认仅存内存**，退出即清；落盘为显式开关（写 `%LOCALAPPDATA%`，可一键清空）。
- 默认**不自动写入本机剪贴板**（避免覆盖当前内容），由「自动镜像」开关开启。
- 仅监听 LAN；图片设大小上限，超限丢弃；坏帧 / 认证失败静默丢弃，后台线程不崩。

### 15.5 设备上限与付费版预留

开源版 `ClipEdition.MaxPeers = 2`（即 1 条远程链路），配对时强制。传输抽象使未来 `RelayTransport`（经服务器中转）可替换 LAN 直连，从而解除上限并支持跨局域网/跨网段——板与 UI 逻辑无需改动。

### 15.6 开放问题

1. **配对记忆**：是否在首次 PIN 配对后保存长期信任（派生长期密钥），免去每次重新输 PIN？（当前为每会话配对，更安全。）
2. **文件类型**：v1 支持文本 + 图片；是否扩展到文件 / 文件列表（接近 FTP 传输工作量）。
3. **PAKE 硬化**：是否引入 SPAKE2/J-PAKE 以抵御在线猜 PIN 的主动中间人。
